...
class TenancyUtilityBill(models.TransientModel):
    ...
    def create_utility_bill(self):
        ...
        for rec in tenancy_ids:
            ...
            if final_dict:
                narration = _('Customer: ') + rec.partner_id.name
                narration = narration + "<br/>" + _('Company: ') + rec.company_id.name
                narration = narration + "<br/>" + _('Reference: ') + rec.name
                narration = narration + "<br/>" + _('Residents: ') + \
                    ','.join([recident.name for recident in rec.resident_ids])
                narration = narration + "<br/>" + _('Utiltiy Period: ') + \
                    self.date_start.strftime(
                        '%m/%d/%Y') + ' - ' + self.date_end.strftime('%m/%d/%Y')
                narration = narration + "<br/>" + _('Tenancy Period: ') + \
                    ten_date_start.strftime(
                        '%m/%d/%Y') + ' - ' + ten_date_end.strftime('%m/%d/%Y')
                narration = narration + "<br/>" + _('Contractual Partners: ') + \
                    ', '.join(
                ...
                if rec.property_id.parent_property_id:
                    property_ids = [rec.property_id.parent_property_id.id]
                else:
                ...
                inv = self.env['account.move'].create({'partner_id': rec.partner_id.id or False, 'ref': rec.name, 'property_ids': [(6, 0, property_ids)],
                                                       'journal_id': journal_id.id, 'move_type': 'out_invoice', 'narration': _(narration), 'tenancy_id': rec.id})
                ...
                for d in final_dict:
                    ...
                    account_id = d.product_id.product_tmpl_id.get_product_accounts()['income']
                    ...
                    desc = _("Cost Type: ") + d.name + " - "
                    original_amount_format = tools.format_amount(self.env, original_amount, inv.currency_id)
                    desc += _("Total Amount: ") + str(original_amount_format) + " - "
                    if d.allocation_formula:
                        desc += _("Allocation Formula: ") + str(allocation_formula)
                    ...
                    inv_line = {'product_id': d.product_id.id, 'product_uom_id': d.product_id.uom_id.id, 'name': _(desc),
                                'move_id': inv.id, 'account_id': account_id.id, 'analytic_account_id': analytic_account,
                                'quantity': 1, 'price_unit': sum_amount, 'tax_ids': tax_ids.ids}
